...
<html lang="es">
  <head>
    ...
    <link rel="stylesheet" href="./css/rigth.css" />
    <link rel="icon" href="./Assets/icons/Uranus.ico" type="image/x-icon" />
    <script src="js/main.js"></script>
  </head>

  <body>
      <!-- Verificación de sesión (simulada con JS, en producción usa PHP) -->
   <body>
<?php
session_start();
error_log("Sesión al cargar home: " . print_r($_SESSION, true));

if (!isset($_SESSION['usuario_id'])) {
    error_log("Redireccionando a login - Sesión no válida");
    header("Location: login.html");
    exit();
}
?>
    <script>
        // Guardar datos de sesión PHP en sessionStorage para JS
        const usuarioData = {
            id: '<?php echo $_SESSION["usuario_id"]; ?>',
            nombre: '<?php echo $_SESSION["usuario_nombre"]; ?>',
            rol: '<?php echo $_SESSION["usuario_rol"]; ?>'
        };
    ...
    <div class="dashboard">
      ...
      <div class="sidebar-left">
        <div class="logo">
          ...
          <button class="btn-view" onclick="window.location.href='tareas.html'">Ver</button>
        </div>
  ...
  <img src="Assets/icons/power-icon.webp" alt="Cerrar sesión" />
</a>
</div>
      </div>
...
<div class="sidebar-right">
      ...
      <div class="submenu">
        <div class="submenu-item has-submenu" onclick="toggleNestedSubmenu(this)">
          ...
          <div class="nested-submenu">
            <a href="/Prueba/Modulos/Vacaciones/index.php" class="card-view-btn">Probar Ruta VACACIONES</a>
  ...
  <div class="card-header">
    <span class="card-title">Prueba Vacaciones</span>
    <div style="display: flex; align-items: center; gap: 10px">
      <span>🧪</span>
      <a href="Modulos/Vacaciones/index.php" class="card-view-btn">Probar Ruta</a>
    </div>
  </div>
</div>
        <div class="card">
    ...
    <script>
      // Cargar datos del usuario desde sessionStorage (simulación)
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Verificar rol y ocultar elementos no permitidos
            verificarRol();
        });

        function verificarRol() {
            const rol = sessionStorage.getItem('usuario_rol') || 'invitado';
            
            // Ocultar elementos según el rol
            document.querySelectorAll('[data-rol]').forEach(elemento => {
                if (elemento.getAttribute('data-rol') !== rol && rol !== 'admin') {
